...
Definition ispalindrome (VP_store: store) := 
	...
	let (VP_store,VP_poison) := 
		(if (negb VP_poison) then
 			 (update VP_store "VP_LEN" (VInteger ( get_int (vp_length VP_store (get_string VP_store "VP_STR")) "VP_result" )),VP_poison) 
		else
 			(VP_store,true)) in
	let (VP_store,VP_poison) := 
		(if (negb VP_poison) then
...
while ( fun VP_store => bounds_op (get_int VP_store "VP_I") (get_int VP_store "VP_LEN" / 2) ) with VP_store upto 1000%nat begin fun VP_store => 			let VP_store := if (subscript (get_string VP_store "VP_STR") (get_int VP_store "VP_I") 0 !=? subscript (get_string VP_store "VP_STR") (get_int VP_store "VP_LEN" - get_int VP_store "VP_I" + 1) 0) then
